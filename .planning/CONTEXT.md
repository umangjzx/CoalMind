...
| Extraction rules | table-driven regex `Spec`s per doc_type (`extraction/rules.py`); `_COL_VAL` value pattern stops at a 2-space column gap / newline so `Mine :` doesn't swallow a trailing `Date :` column |
